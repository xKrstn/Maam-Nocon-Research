<?php 
    ...
    $id=$_SESSION['id'];
    $select="SELECT * FROM customer_tbl WHERE customerID='$id'";
        ...
        <form  method="post" style="width:25vw; min-width:30px;">
            ...
            <?php 
                if(isset($_POST['submit'])){
                    ...
                    $query= "UPDATE customer_tbl SET firstname='$firstname',lastname='$lastname', phonenum='$phonenum' WHERE customerID='$id'";
            
                    if ($con->query($query)===TRUE){
                        echo "<script>window.location='usersettings.php?msg=Account Updated Successfully';</script>";
                    } else {
            ...
            <?php 
                if(isset($_POST['change'])){
                    ...
                    if(password_verify($currentpass,$hashpass)){
                        if($password !== $conpass){
                            echo '<script> window.location.href="usersettings.php?msg=Password and Confirm Password doesnt match";</script>';
                        }
                        else{
                            ...
                            if ($con->query($query)===TRUE){
                                echo "<script>window.location='usersettings.php?msg=Password Updated Successfully';</script>";
                            } else {
                                echo "Failed: " . mysqli_error($con);
                            }
                        }
                        ...
                        echo '<script> window.location.href="usersettings.php?msg=Invalid Current Password";</script>';
                    }
